...
Using google syntax but maybe should be using sphinx? 
https://pythonhosted.org/an_example_pypi_project/sphinx.html

    """ Short Description 

    Args:
	obj (bigip_obj): bigip connection object.
...
############ RE:  LTCONFIG METHODS    ######################
# SHOULD TRY TO AVOID LTCONFIG METHODS WHENEVER POSSIBLE 
# BUT SOMETIMES THEY ARE UNAVOIDABLE.
############################################################

def set_LTConfig_field_values ( obj, class_name, container, container_class, fields_values ):
	...
	class_name (string): class name. 
	    Note: Possible Classes for LT Class on v11.5.1
	...
	container_class (string): container class name.
	field_values (dict): dict containing field names & values
	    ex.
	    { 'field_name_1' : value_1 , 'field_name_2': value_2, 'field_name_3' : value_3 }
	...
	for k,v in fields_values.items():
        ...
        print "Set " + class_name + " config error. Check log."
        traceback.print_exc(file=sys.stdout)



def get_LTConfig_field_values ( obj, class_name, container, container_class, fields ):

    """ Used simply to confirm if associated set_method worked. Prints and returns the values of the fields. 
	Just a simple wrapper for the Loosely Typed Config get_value methods. 
	...
	container (string):  container name. 
	container_class (string):  container class name. 
	fields (dict): dictionary of field_names 
	    ex.
	    { 'field_name_1' : value_1 , 'field_name_2': value_2, 'field_name_3' : value_3 }

		       Note: dict will include values (since mostly just reusing same structure as with set_field_values cmd). 
			     Will simply ignore values though.
	 
    Returns:
       array.  The same output provided by LTConfig.Field.get_values cmd. 

    Raises:

    """
    try:
	class_instance_key = { 'name' : class_name , 
			       'class_name' : class_name, 
# 			       'container' : container, 
#			       'container_class' : container_class, 
			     } 
	...
	for k,v in fields.items():
        ...
        print "Get " + class_name + " config error. Check log."
        traceback.print_exc(file=sys.stdout)
...
#  These are duplicate of LTConfig methods below
...
def set_snmp_config_client_access (obj, networks, masks):
    ...
    try:
        ...
        for i in range(len(networks)):
            ...
            client_access_seq.append(client_access_obj)

        obj.Management.SNMPConfiguration.set_client_access( client_access_info = client_access_seq )

    except:
        print "Set SNMP config error. Check log."
        traceback.print_exc(file=sys.stdout)

def get_snmp_config_client_access (obj):

    try:
       
	# network_output looks like: 
	# [{'netmask': '255.255.0.0', 'address': '10.11.0.0'}, {'netmask': '0.0.0.0', 'address': '127.0.0.0'}]
 
        networks_output = obj.Management.SNMPConfiguration.get_client_access()

        for i in range(len(networks_output)):
	    print networks_output[i]['address'] + "/" + networks_output[i]['netmask']
...
def set_snmp_config_trap_dests (obj, sink_type, sink_hosts, sink_ports, sink_communities ):
    ...
    try:
        ...
        for i in range(len(sink_hosts)):
            sink_info_obj =	{ 
				'sink_host' 	: sink_hosts[i],
				'sink_port' 	: sink_ports[i],
				'sink_community' 	: sink_communities[i],
				}
            sink_info_obj_seq.append(sink_info_obj)
        

        obj.Management.SNMPConfiguration.set_trap_sinks( sink_type = sink_type , sink_info = sink_info_obj_seq )
...
def get_snmp_config_trap_dests ( obj, sink_type ):

    try:
        
	#trap output looks like:
	# [{'sink_host': '10.11.31.1', 'sink_port': 167, 'sink_community': 'XXXXXXX'}, {'sink_host': '10.10.31.2', 'sink_port': 167, 'sink_community': 'YYYYYYY'}]

        trap_output = obj.Management.SNMPConfiguration.get_trap_sinks( sink_type = sink_type )
	

        for i in range(len(trap_output)):
	    print "Trap: " 
	    print "	Host:      " + trap_output[i]['sink_host']
	    print "	Port: 	   " + str(trap_output[i]['sink_port'])
	    print "	Community: " + trap_output[i]['sink_community']

        return trap_output

    except:
        print "Get Trap Dest config error. Check log."
        traceback.print_exc(file=sys.stdout)



### ALTERNATIVE LTConfig Methods
 
def set_snmp_ltconfig ( obj, fields ):

    """
	    # >>> b.LTConfig.Field.get_list(['snmpd'])
	    # [['agent_address', 'agenttrap', 'allow', 'authtrapenable', 'bigip_traps', 'description', 
		'include', 'l2forward_vlan', 'load_max1', 'load_max15', 'load_max5', 'syscontact', 
		'syslocation', 'sysservices', 'trapcommunity', 'trapsource']]
...
def get_snmp_ltconfig ( obj, fields ):
...
def set_snmp_trap_dest_ltconfig ( obj, fields_values ):

    """
    field_values input should be dict like:
    {
	'10.11.31.1': {'host': '10.11.31.1', 'port': '162', 'community': 'my_community', 'version': '2c'}, 
	'10.11.31.2': {'host': '10.11.31.2', 'port': '162', 'community': 'my_community', 'version': '2c'}
    }

        # Instead of using the specific v1 or v2 trapsink classes, looks like need to create a new sub class
        # for each trapsink object under trapsess class
        # appears to be equiv of:
        # https://devcentral.f5.com/wiki/iControl.Management__SNMPConfiguration__set_generic_traps_v2.ashx

        # >>> snmp_trapsess_instances = b.LTConfig.Class.get_list_of_instances(['trapsess'])
        # >>> snmp_trapsess_instances
        # [[{'class_name': 'trapsess', 'container': '/Common/snmpd', 'name': '/Common/i10_11_31_1_1', 'container_class': None},
        #  {'class_name': 'trapsess', 'container': '/Common/snmpd', 'name': '/Common/i10_11_31_2_1', 'container_class': None}]]

        # >>> snmp_trapsess_fields = b.LTConfig.Field.get_list(['trapsess'])
        # >>> snmp_trapsess_fields
        # [['auth_password', 'auth_protocol', 'community', 'description', 'engine_id',
        # 'host', 'port', 'privacy_password', 'privacy_protocol', 'security_level', 'security_name', 'version']]

        # >>> b.LTConfig.Field.get_values( snmp_trapsess_classes[0], [ snmp_trapsess_fields[0], snmp_trapsess_fields[0] ] )
        # [['', 'NONE', 'my_community', '', '', '10.11.31.1', '162', '', 'NONE', 'noAuthNoPriv', '', '2c'],
        #  ['', 'NONE', 'my_community', '', '', '10.11.31.2', '162', '', 'NONE', 'noAuthNoPriv', '', '2c']]
	...
	for k,v in fields_values.items():
	    #BIGIP Default Naming Convention
            class_name_field = "i" + k.replace(".", "_") + "_1"
            snmp_trapsess_class = { 'class_name': 'trapsess', 'container': '/Common/snmpd', 'name': class_name_field , 'container_class': None }
            class_instance_keys.append(snmp_trapsess_class)

	    trap_fields = []
	    trap_values = []
	    for i,j in v.items():
		#print "%-30s" % i + "%s" % j
		trap_fields.append(i)
		trap_values.append(j)
	    field_instance_names.append(trap_fields)
	    values.append(trap_values)
...
def get_snmp_trap_dest_ltconfig ( obj, traps ):

    """
    field structure looks similar to structure in "set_snmp_trap" but will just send the field names out of structure:
    {
	'10.11.31.1': {'host': 'XXXXX', 'port': 'XX', 'community': 'XXXXX', 'version': 'X'}, 
	'10.11.31.2': {'host': 'XXXXX', 'port': 'XX', 'community': 'XXXXX', 'version': 'X'}
    }
    """
	...
	for k,v in traps.items():
        ...
        print "%-30s" % "field" + "%s" % "value"
        print "%-30s" % "-----" + "%s" % "-----"
        for c in range(len(field_instance_names)):
            print "Trap Dest Name = " + class_instance_keys[c]['name'] + ":"
            for i in range(len(field_instance_names[c])):
                print "%-30s" % field_instance_names[c][i].strip() + "%s" % values_output[c][i].replace('\n','\n\t\t\t\t').strip()
        return values_output

    except:
        print "Get SNMPD trap dest config error. Check log."
        traceback.print_exc(file=sys.stdout)



def set_radius_servers (obj, radius_servers ):
    
    # radius_servers input looks like:
    # { 'system_auth_name2': {'ip': '10.11.26.2', 'secret': 'abc123@@', 'port': '1812', 'timeout': '6'}, 
    #	'system_auth_name1': {'ip': '10.11.26.1', 'secret': 'abc123@@', 'port': '1812', 'timeout': '6'} }

    try:

	radius_server_seq = []

	for k,v in radius_servers.items():
	    ...
	    radius_server_seq.append( radius_server_obj )
...
def set_radius_config (obj, config_name , radius_server_names  ):

    # Method only creates one config name with multiple servers 

    try:

        obj.Management.RADIUSConfiguration.create( config_names = [ config_name ] , servers = [ radius_server_names ] )
...
def set_tacacs_config (obj, tacacs_servers ):

    try:

   
	#Hardcoding config name as really only dealing with system auth 
	config_name_seq = [ "system-auth" ]
  
        secrets_seq = []
        service_names_seq = []
        protocol_names_seq = []
	tacacs_server_ips_seq = []

	for k,v in tacacs_servers.items():
	    secrets_seq.append(v['secret'])
            service_names_seq.append(v['service_name'])
            protocol_names_seq.append(v['protocol_name'])
            tacacs_server_ips_seq.append(v['address'])
	    
	tacacs_server_ips_seq_seq = [ tacacs_server_ips_seq ]
...
def set_system_auth_source ( obj, auth_source ):

    try:
	obj.Management.UserManagement.set_authentication_method( auth_method = auth_source )  

    except:
    
        print "Set System Auth method error. Check log."
        traceback.print_exc(file=sys.stdout)
...
def set_system_auth_default_console_access( obj, system_auth_console ):
    ...
    # To set the shell, we'll have to use LTConfig method 
    try:
...
def set_system_auth_default_shell( obj, system_auth_shell ):
    # Unfortunately, this method doesn't allow you to set shell for special remote-user defaults.

    # root@(bigip-1)(cfg-sync In-Sync)(Active)(/Common)(tmos)# list auth remote-user all-properties
    # auth remote-user {
...
def set_system_auth_remote_users_ltconfig( obj, fields ):

    #LTConfig version
    # >>> fields
    # [['default_partition', 'default_role', 'description', 'remote_console_access']]
    # >>> b.LTConfig.Field.get_values( [{ 'class_name' : "remote_users", 'name' : "remote_users" }], fields )
    # [['[All]', 'none', '', 'tmsh']]
    ...
    try:
        ...
        print "Set Remote Users LTconfig error. Check log."
        traceback.print_exc(file=sys.stdout)

def get_system_auth_remote_users_ltconfig( obj, fields ):
    #LTConfig version
    try:
        get_LTConfig_field_values ( obj, 'remote_users', 'None', 'None', fields )

    except:
        print "Get Remote Users LTConfig error. Check log."
        traceback.print_exc(file=sys.stdout)
...
def set_remote_roles( obj, roles ):
    ...
    try:
	# not using LTConfig wrapper methods for now. 
	...
	for k,v in role_names_dict.items():
		...
		class_instance_keys.append(class_instance_key)
	        #Just use first one as a structure for names. Requires they're all the same
		field_names = []
		field_values = []
		for i,j in v.items():
...
def set_self_ips( obj, names, addresses, netmasks, vlan_names, traffic_groups, floating_states, port_lock_down_list, custom_ports ):
    try:
        ...
        # Default  + BGP  ### NOT WORKING? ### Can try "PROTOCOL_ANY" instead of 'PROTOCOL_UNKNOWN'
...
def set_gateway_pools (obj, self_ips, self_ips_netmasks, vlans ):
	...
	# really should create use 2 transparent gateway monitors
	# i.e. SOL6848: Monitoring link objects on a BIG-IP GTM or BIG-IP Link Controller system	
...
def set_profile_enabled_state ( obj, value ):
    ...
    #Bunch of misc short defs for translating/setting profile settings
...
def set_ProfileHardwareAccelerationMode (obj, value):
    ...
    try:
        ...
        if value == 'none':
            value = "HW_ACCELERATION_MODE_NONE"
        if value == 'assist':
            value = "HW_ACCELERATION_MODE_ASSIST"
        if value == 'full':
            value = "HW_ACCELERATION_MODE_FULL"
...
def set_ProfileTCPOptionMode (obj, value): 
    ...
    try:
        ...
        if value == 'preserve':
            value = "TCP_OPTION_MODE_PRESERVE"
        if value == 'rewrite':
            value = "TCP_OPTION_MODE_REWRITE"
        if value == 'strip':
            value = "TCP_OPTION_MODE_STRIP"
	
	ProfileTCPOptionMode  =	{
					    'default_flag' :  bool(0),
					    'value' : value
					}
...
def set_ProfileTCPCongestionControlMode (obj, value): 
    ...
    try:
        ...
        if value == 'highspeed':
            value = "TCP_CONGESTION_CONTROL_HIGHSPEED"
        if value == 'newreno':
            value = "TCP_CONGESTION_CONTROL_NEWRENO"
        if value == 'reno':
            value = "TCP_CONGESTION_CONTROL_RENO" 
        if value == 'scalable':
            value = "TCP_CONGESTION_CONTROL_SCALABLE" 
        if value == 'none':
            value = "TCP_CONGESTION_CONTROL_NONE"

        ProfileTCPCongestMode  =	{
					    'default_flag' :  bool(0),
					    'value' : value
					}

        return ProfileTCPCongestMode
...
#
	...
	if value == 'carp':
	    value = "PERSISTENCE_HASH_CARP"
	if value == 'none':
	    value = "PERSISTENCE_HASH_NONE"
...
# }
	...
	# here appears to be a few settings that are order sensitive
	# ex. proxy buffer settings
	# error_string : 01070734:3: Configuration error: The proxy buffer low watermark must be less than or equal to the proxy buffer high watermark in TCP profile (/Common/tcp_custom)'
	# So set them first
	for key,value in dict.items():
	    if key == 'proxy buffer high':
	        profileUlong = set_profile_ULong( obj, value )
...
# }
    ...
    try:
        ...
        persist_mode = "PERSISTENCE_MODE_SOURCE_ADDRESS_AFFINITY"
...
def main():
    ...
    parser.add_option("-f", "--license_file", action="store", type="string", dest="local_license_file_name")
    parser.add_option("-e", "--eula", action="store", type="string", dest="local_eula_file_name")
    parser.add_option("-l", "--license", action="store_false", dest="license", default=True)
    (options, args) = parser.parse_args()

    ################### LOAD MOST OF CONFIG VARIABLES FROM FILE  ##############################
    # As BIGIP's configs are a little complex and frequently changing, original intent was to simplify as much as possible for consistency 
    # down to key,value pair or more universal .ini style config, with intention of eventually storing in an actual database 
    # Appeared that the most popular module to parse that was the configobj module
    # http://www.voidspace.org.uk/python/configobj.html
    # which appeared to be preferred over builtin configparser module
    ...
    #if we want to grab password from prompt instead comment out line below
...
############### START PROCESSING LOGIC  ##################
    ...
    # If license option flag is set. Default = True.
...
#     get_system_config ( b, sys_fields ) 
# 
    ...
    snmp_sys_name = snmp_dict['snmp_sys_name']
    snmp_sys_location = snmp_dict['snmp_sys_location']
    snmp_sys_contact = snmp_dict['snmp_sys_contact']
    snmp_sys_description = snmp_dict['snmp_sys_description']
    snmp_sys_object_id = snmp_dict['snmp_sys_object_id']
    snmp_sys_services =  snmp_dict['snmp_sys_services']

    snmp_client_access_dict = dict(config['SYSTEM']['SNMP']['SNMP_CLIENT_ACCESS'])
    snmp_client_networks = []
    snmp_client_masks = []
    for k,v in snmp_client_access_dict.items():
 	snmp_client_networks.append( v['network'] )
 	snmp_client_masks.append( v['mask'] )

    snmp_trap_dest_dict = dict(config['SYSTEM']['SNMP']['SNMP_TRAP_DESTS'])
    snmp_sink_type = "SINK_TRAP2SINK"
    trap_dest_hosts = []
    trap_dest_ports = []
    trap_dest_communities = []
    for k,v in snmp_trap_dest_dict.items():
 	trap_dest_hosts.append( v['host'] )
 	trap_dest_ports.append( v['port'] )
 	trap_dest_communities.append( v['community'] )
    ...
    print "Setting SNMP Client Access: "
    set_snmp_config_client_access( b , snmp_client_networks, snmp_client_masks)
    print "Getting SNMP Client Access: "
    get_snmp_config_client_access( b )

    print "Setting SNMP Trap Destinations: "
    set_snmp_config_trap_dests( b , snmp_sink_type, trap_dest_hosts, trap_dest_ports, trap_dest_communities )
    print "Getting SNMP Trap Destinations: "
    get_snmp_config_trap_dests( b , snmp_sink_type  )


#     Using Alternate LTCONFIG Methods
#     snmp_dict = dict(config['SYSTEM']['SNMP']['SNMP_VARS_LTCONFIG'])
#     snmp_fields = { 
# 		    'allow'	:  snmp_dict['allow'],
# 		    'description' : snmp_dict['description'],
# 		    'include' : snmp_dict['include'],
# 		    'syscontact' : snmp_dict['syscontact'],
# 		    'syslocation' : snmp_dict['syslocation'],
# 		    'trapcommunity' : snmp_dict['trapcommunity'],
# 		  } 
# 
#     snmp_trap_dests_dict = dict(config['SYSTEM']['SNMP']['SNMP_TRAP_DESTS_LTCONFIG'])
#     snmp_trap_dest_fields = snmp_trap_dests_dict
#     trap_dest_hosts = []
#     trap_dest_ports = []
#     trap_dest_communities = []
#     trap_dest_versions = []
# 
#     for k,v in snmp_trap_dests_dict.items():
# 	trap_dest_hosts.append( v['host'] )
# 	trap_dest_ports.append( v['port'] )
# 	trap_dest_communities.append( v['community'] )
# 	trap_dest_versions.append( v['version'] )
# 
#     print "Setting SNMP Config:" 
#     set_snmp_ltconfig ( b, snmp_fields )
#     print "Getting SNMP Config:" 
#     get_snmp_ltconfig ( b, snmp_fields ) 
#     
#     print "Setting SNMP Trap Destinations Config" 
#     set_snmp_trap_dest_ltconfig( b, snmp_trap_dest_fields ) 
#     print "Getting SNMP Trap Destinations Config"  
#     get_snmp_trap_dest_ltconfig ( b, snmp_trap_dest_fields )
# 
# 

    ###### SYSTEM AUTH SETTINGS #######
    print "\nBegin Configuring System Auth: "

#     ######  RADIUS  ####### 
# 
#     print "Setting RADIUS SERVERS:"
#     radius_servers_dict = dict(config['SYSTEM']['SYSTEM_AUTH']['RADIUS'])
# 
#     radius_server_names = []
#     for k in radius_servers_dict:
# 	radius_server_names.append(k)
# 
#     set_radius_servers ( b, radius_servers_dict )  
#     
#     #print "Setting RADIUS CONFIG:"
#     set_radius_config( b, "system-auth", radius_server_names )  
# 
#     ###### END RADIUS #######
#  
# 
#     ###### TACACS ####### 
#     
#     print "Setting TACACS SERVERS"
#     tacacs_servers_dict = dict(config['SYSTEM']['SYSTEM_AUTH']['TACACS'])
# 
#     set_tacacs_config( b, tacacs_servers_dict )
#     
#     ###### END TACACS ####### 
 
    system_auth_dict = dict(config['SYSTEM']['SYSTEM_AUTH'])
    system_auth_fields = {
			    'default_role' 		: system_auth_dict['default_role'],
			    'default_partition' 	: system_auth_dict['default_partition'],
			    'remote_console_access'	: system_auth_dict['remote_console_access'],
			}

    print "Setting SYSTEM AUTH SOURCE:"
    set_system_auth_source (b, system_auth_dict['system_auth_source'] )  

    #Methods use official Management.UserManagement methods
    #set_system_auth_default_role( b, system_auth_dict['system_auth_role'] )
    #set_system_auth_default_partition( b, system_auth_dict['system_auth_partition'] )
    #set_system_auth_default_shell( b, system_auth_dict['system_auth_shell'] )    
    #set_system_auth_default_console_access( b, system_auth_dict['system_auth_console'] )  
    ### However, doesn't quite work for remote_user so need to use LTconfig below ####

    #LTConfig Methods:
    set_system_auth_remote_users_ltconfig( b, system_auth_fields )    
    set_system_auth_remote_users_ltconfig( b, system_auth_fields ) 
    ...
    # SET REMOTE ROLES:
    # Using AD
    remote_roles_dict = dict(config['SYSTEM']['SYSTEM_AUTH']['REMOTE_ROLES']['AD'])
#     # Using RADIUS
#     remote_roles_dict = dict(config['SYSTEM']['SYSTEM_AUTH']['REMOTE_ROLES']['RADIUS'])
    roles = {}
...
# 
    ...
    # Base Profiles
    print "\n\nSETTING BASE PROFILES:"

    fastl4_profiles_dict = dict(config['TRAFFIC']['PROFILES']['FASTL4'])
    tcp_profiles_dict = dict(config['TRAFFIC']['PROFILES']['TCP'])
    udp_profiles_dict = dict(config['TRAFFIC']['PROFILES']['UDP'])
    http_profiles_dict = dict(config['TRAFFIC']['PROFILES']['HTTP'])
    oneconnect_profiles_dict = dict(config['TRAFFIC']['PROFILES']['ONECONNECT'])
    src_persist_profiles_dict = dict(config['TRAFFIC']['PROFILES']['SRC_PERSIST'])

    #NOTE: If creating child profiles that inherit from custom profiles, will need to send in order instead.
    print "\n\nSetting FASTL4 Profiles:"
    for k,v in fastl4_profiles_dict.items():
	print "    Setting profile: " + k
	set_fastl4_profile( b, k, v ) 
    print "\nGetting FASTL4 Profiles:"
    for k,v in fastl4_profiles_dict.items():
	get_fastl4_profile( b, k )

    print "\n\nSetting TCP Profiles:"
    for k,v in tcp_profiles_dict.items():
	print "    Setting profile: " + k
	set_tcp_profile( b, k, v ) 
    print "\nGetting TCP Profiles:"
    for k,v in tcp_profiles_dict.items():
	get_tcp_profile( b, k )

    print "\n\nSetting UDP Profiles:"
    for k,v in udp_profiles_dict.items():
	print "    Setting profile: " + k
	set_udp_profile( b, k, v ) 
    print "\nGetting UDP Profiles:"
    for k,v in udp_profiles_dict.items():
	get_udp_profile( b, k )

    print "\n\nSetting HTTP Profiles:"
    for k,v in http_profiles_dict.items():
	print "    Setting profile: " + k
	set_http_profile( b, k, v ) 
    print "\nGetting HTTP Profiles:"
    for k,v in http_profiles_dict.items():
	get_http_profile( b, k )

    print "Setting ONECONNECT Profiles:"
    for k,v in oneconnect_profiles_dict.items():
	print "    Setting profile: " + k
	set_oneconnect_profile( b, k, v ) 
    print "\nGetting ONECONNECT Profiles:"
    for k,v in oneconnect_profiles_dict.items():
	get_oneconnect_profile( b, k )

    print "\n\nSetting SRC PERSIST Profiles:"
    for k,v in src_persist_profiles_dict.items():
	print "    Setting profile: " + k
	set_src_persist_profile( b, k, v ) 
    print "\nGetting SRC PERSIST Profiles:"
    for k,v in src_persist_profiles_dict.items():
	get_src_persist_profile( b, k )
